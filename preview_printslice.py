...
def read_images_from_folder(folder_path):
    """
    按照文件名排序读取文件夹中的所有图片文件
    
    参数:
        folder_path: 包含图片的文件夹路径
    
    返回:
        imgs: 包含所有图片的numpy数组，形状为(n, height, width, channels)
    """
    # 获取文件夹中所有图片文件
    ...
    imgs = np.zeros((len(image_files), height, width, channels), dtype=np.float16)
    
    # 读取所有图片并转换颜色空间
    print(f"Reading {len(image_files)} images from {folder_path}...")
    for i, file in enumerate(tqdm(image_files)):
        ...
        imgs[i, :, :, :] = img_rgb.astype(np.float16)
