...
#     # 获取所有 .npz 文件路径，按批次排序
#     npz_files = sorted(
#         [os.path.join(folder_path, file) for file in os.listdir(folder_path) if file.endswith(".npy")]
#     )
...
#         batch_data = np.load(npz_file)
#         imgs = batch_data['arr_0']  # 根据保存时的键名读取数组
        
#         # 转换 RGB 通道为 BGR
#         for i in range(imgs.shape[0]):
#             imgs[i, :, :, 0:3] = cv2.cvtColor(imgs[i, :, :, 0:3], cv2.COLOR_RGB2BGR)
        
#         # 将当前批次添加到总图像列表
#         all_imgs.append(imgs)
    
#     # 将所有批次拼接成一个大的 ndarray
#     all_imgs = np.concatenate(all_imgs, axis=0)
    
#     # 对结果进行翻转并返回
#     return np.flip(all_imgs, axis=0)

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--input_folder', type=str, default="/media/vrlab/rabbit/print3dingp/workspace/experiment/lego_figure/print")
